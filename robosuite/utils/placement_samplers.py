...
class AbsolutePositionSampler(ObjectPositionSampler):
    """
    Places all objects within the absolute pose.
    ...
    example:
        ...
        if self.placement_initializer is not None:
            obj_pose = [[0.0, 0.25, 0.83, 1.0, 0, 0, 0.], #wxyz
                    [0.25, 0.25, 0.83, 1, 0, 0, 0]]
            # Create placement initializer
            self.placement_initializer = AbsolutePositionSampler(name = "ObjectSampler")
            self.placement_initializer.reset()
            self.placement_initializer.add_absolute_pose(obj_pose)
    ...
    def __init__(
        ...
        reference_pos=(0, 0, 0),
        z_offset=0.0,
        # absolute_pos = [(0.0658, -0.07578, 0.83), (-0.024671001792052928, -0.010126262226524615, 0.83)],
        # absolute_quat = [np.array([0.53932708, 0., 0., 0.84209637]), np.array([0.53932708, 0., 0., 0.84209637])],
        absolute_pos = [],
        absolute_quat = [],
    ):
        self.x_range = x_range
        ...
        self.absolute_quat = absolute_quat
    ...
    def add_absolute_pose(self, pose=[]):
        """
        Add absolute pose

        Args:
            pose (list of list): multi poses, [x, y, z, qw, qx, qy, qz]
        ...
        for it in pose:
            pos = (it[0], it[1], it[2])
            qua = np.array([it[3], it[4], it[5], it[6]])
            self.absolute_pos.append(pos)
            self.absolute_quat.append(qua)
    ...
    def _sample_y(self, object_horizontal_radius):
        """
         s the y location for a given object
    ...
    def sample(self, fixtures=None, reference=None, on_top=True):
        ...
        # import ipdb; ipdb.set_trace()
        for i in range(0, len(self.mujoco_objects)):
            placed_objects[self.mujoco_objects[i].name] = (self.absolute_pos[i], self.absolute_quat[i], self.mujoco_objects[i])
